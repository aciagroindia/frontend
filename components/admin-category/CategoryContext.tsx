...
export function CategoryProvider({ children }: { children: ReactNode }) {
  ...
  const normalizeCategory = (category: any): Category => ({ // Corrected type annotation
    ...category,
    id: category._id,
  });
  ...
  const addCategory = async (formData: FormData): Promise<boolean> => {
    try {
      ...
      if (response.data.success) {
        ...
        // Perf Optimization: Update state directly instead of a full refetch
        const newCategory = normalizeCategory(response.data.data);
        setCategories((prev) => [newCategory, ...prev]);
        return true;
      }
      // Bug Fix: Handle API error case where success is false
      toast.error(response.data.message || 'Category jodne mein samasya aayi.');
      return false;
    } catch (err: any) {
  ...
  const updateCategory = async (id: string, formData: FormData): Promise<boolean> => {
    try {
      const response = await axiosInstance.put(`/categories/${id}`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      // Bug Fix: Check for success field in response
      if (response.data.success) {
        ...
        toast.error(response.data.message || 'Category update karne mein samasya aayi.');
        return false;
      }
    } catch (err: any) {
